...
import { getPostDraft, unpublishPost } from '../../../../../server/cms/repositories/posts.ts';
...
export const POST: APIRoute = async ({ request, locals, params }) => {
  ...
  try {
    ...
    const post = await unpublishPost(db, postId, expectedDraftVersion);
      ...
      try {
        ...
        return cmsErrorResponse(new CmsConflictError(error.message, error.code, {
          details: {
            ...
            // unpublishPost only ever matches lifecycle = 'active'; when the
            // row is already 'draft'/'archived' no draft-version retry can
            // ever succeed, so the client needs this to stop suggesting one.
