...
| Item | Status |
|---|---|
| Completed | P0-01 through P0-04; Gate G0; S-01 through S-06; Gate G0.5; P1-00 through P1-03; P1-API-01; 1C-01; 1C-02; 1C-03; 1C-06; P1-04a; 1C-07 |
...
| Real blocker (2026-09-12, current) | **The staging hostname `cms-staging.frong.me` does not exist.** It returns NXDOMAIN, so the release callbacks never reach the Worker: run 34667686189 fired both callback steps and both died with `curl: (6) Could not resolve host: cms-staging.frong.me`. The Worker is live only at `https://frong-me-staging.frongbook.workers.dev`, which is **not** fronted by Cloudflare Access (no Access redirect or cookie headers; the 401 seen there is our own middleware rejecting a request with no `Cf-Access-Jwt-Assertion`). Because Access never sees the request, the workflow's `CF-Access-Client-Id`/`CF-Access-Client-Secret` service-token headers cannot be exchanged for a JWT assertion, so simply repointing the callback at the workers.dev host would still return 401. Owner action: create the Workers custom domain and put the Access application in front of it with a Service Auth policy for that service token. `CMS_STAGING_HOST` in the workflow is now overridable via repo/environment `vars` so the host can be corrected without a code change. Note this also contradicts the Gate G0.5 evidence, which recorded `https://cms-staging.frong.me` as an owner-verified running staging Worker — that hostname is not resolving now |
| Related (unverified) | **The deployed Worker's secrets remain unconfirmed.** Two consecutive deploy binding tables (runs 34667010772, 34667558634) list only `DB`, `MEDIA_BUCKET`, and `ASSETS`, with no secret bindings, even though the owner reports running `wrangler secret put --env staging` for all three. This could not be settled from here, because the callbacks never reached the Worker and `/earth*` is gated before any route runs. Definitive check: `npx wrangler secret list --env staging` |
| Earlier blocker (resolved by the above) | **The deployed staging Worker has no environment secrets or vars.** Run 34667010772's binding table lists exactly three: `DB` (D1), `MEDIA_BUCKET` (R2), `ASSETS`. `CF_ACCESS_TEAM_DOMAIN`, `CF_ACCESS_AUD`, `CF_ACCESS_ALLOWED_EMAIL`, and `RELEASE_CALLBACK_SECRET` are all absent, because `wrangler.jsonc` declares no `vars` and GitHub Environment values reach only the CI job, never the Worker runtime. Consequences: the Access guard's `required()` throws on the missing team domain, so `/earth*` rejects **every** request with 403 — including a genuinely valid Access JWT — and the P1-04a `/confirm` and `/fail` routes would answer 503 (fail-closed, as designed) even if a caller got past Access. Both behaviours are safe but non-functional. Fix is owner-side and cannot be done from this workspace (it requires the secret values): `npx wrangler secret put <NAME> --env staging` for each, noting that `--env staging` is required or the secret lands on the `frong-me` Worker instead of `frong-me-staging` |
...
| Routine checks | `npm run test:cms` (177 tests, all passing at this update); `npx wrangler d1 migrations apply DB --local`; `node scripts/db/verify-staging.mjs --wrangler --local` |
