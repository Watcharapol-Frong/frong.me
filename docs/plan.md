...
| Real blocker (2026-09-12) | The staging Worker now deploys successfully (run 34666143593, version `76b236e9-d46c-40f2-9970-9dc7a409559d`, live on `https://frong-me-staging.frongbook.workers.dev`), but **every `/earth*` route returns HTTP 500** — including `/earth`, `/earth/api/posts`, and `/earth/api/releases/[id]/confirm`, with or without a `Cf-Access-Jwt-Assertion` header. Other SSR routes are healthy (`/articles/does-not-exist` → 404, `/` → 200), so this is specific to the `/earth` route tree, not the adapter or the middleware module as a whole. Expected behaviour is 401 from the Access guard. Until this is fixed, the P1-04a confirm/fail callbacks cannot be exercised live at all. Next diagnostic: `npx wrangler tail --env staging` (needs Cloudflare credentials this workspace does not have) or Workers observability, to capture the actual runtime exception |
